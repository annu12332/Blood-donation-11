...
const MyDonationRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(`https://blood-donation-backentd-11.vercel.appdonation-requests/${user.email}`);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.delete(`https://blood-donation-backentd-11.vercel.appdonation-requests/${id}`);
